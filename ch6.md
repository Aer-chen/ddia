...
> 笔记：分区方案的选择可以类比索引（也可以说就是索引)
>
> - 范围分区：根据主键的范围进行分区-利于范围查找，但分区性能较差
> - 散列分区：将主键进行散列之后进行分区，分区性能较好，但是不利于范围查找
...
> 动态分区：思想与B+树一模一样
>
> 思想与B+树类似，节点之间可以合并和分裂。也就是分区可以合并和分裂。
>
> 然后再加上一个预分割的措施使得开始数据不要太集中
...
对于HBase来说，实质上它的数据存储激素分区存储。

分区方法使用了 根据主键范围进行分区

分区节点数量使用了动态分区

这两个思想结合起来实质上和B+树的思想一样。

单个RegionServer相当于B+树的叶子节点，Region相当于B+树的数据项。当Region数据量过大时，就会进行分裂。
